...
ifneq ($(GnuNoInstall),YES)
	...
	./merge-lipo `for arch in $(RC_ARCHS) ; do echo $(BuildDirectory)/install-$$arch ; done` $(DSTROOT)
	 $(FIND) $(DSTROOT) $(Find_Cruft) | $(XARGS) $(RMDIR)
	$(FIND) $(SYMROOT) $(Find_Cruft) | $(XARGS) $(RMDIR)
ifneq ($(GnuNoChown),YES)
	$(CHOWN) -R $(Install_User):$(Install_Group) $(DSTROOT) $(SYMROOT)
endif
endif
ifdef GnuAfterInstall
	 $(MAKE) $(MMAKEFLAGS) $(GnuAfterInstall)
endif
	 if [ -d "$(DSTROOT)$(Workaround_3678855)" ]; then \
